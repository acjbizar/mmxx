...
const App: React.FC = () => {
  ...
  const [glyphChar, setGlyphChar] = useState('A');
...
${polygonsMarkup}
  ...
  const downloadSvg = () => {
    const codepoint = glyphChar.length > 0 
      ? glyphChar.charCodeAt(0).toString(16).toUpperCase().padStart(4, '0') 
      : '0000';
    ...
    a.href = url;
    a.download = `character-u${codepoint}.svg`;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 overflow-hidden">
      {/* Header */}
      <header className="h-16 border-b bg-white flex items-center justify-between px-6 shrink-0 shadow-sm z-20">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-indigo-600 rounded-lg">
              <Type className="text-white w-5 h-5" />
            </div>
            <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent hidden sm:block">
              TypeGrid
            </h1>
          </div>

          <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Char</span>
            <input 
              type="text" 
              maxLength={1}
              value={glyphChar}
              onChange={(e) => setGlyphChar(e.target.value)}
              className="w-8 bg-transparent border-none focus:ring-0 font-mono font-bold text-indigo-600 text-center outline-none p-0"
              title="Current working character"
            />
          </div>
        </div>
      ...
      <main className="flex-1 flex overflow-hidden">
        {/* Workspace - Sticky/Anchored to top */}
        <div className="flex-1 relative bg-slate-100/50 overflow-auto scroll-smooth transition-all">
          <div className="flex flex-col items-center min-h-full">
            {/* The Editor Container */}
            <div className="mt-8 mb-24 px-12 pt-8">
              <div className="bg-white p-12 shadow-2xl rounded-2xl border border-slate-200 ring-4 ring-slate-100/50">
                <svg
                  width={SVG_SIZE * 2}
                  height={SVG_SIZE * 2}
                ...
                >
                  ...
                  {showGridLines && (
                    <g fill="none" stroke="#1e90ff" strokeWidth="0.5" shape-rendering="crispEdges" opacity="0.3" pointerEvents="none">
            ...
            <div className="absolute bottom-6 left-6 text-[10px] text-slate-400 font-bold uppercase tracking-widest bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-full border border-slate-200/50 shadow-sm pointer-events-none">
              Glyph: {glyphChar} | Active: {activeIds.size} / 256
            </div>
          </div>
        </div>

        {/* Side Panel (Code/Export) - Persistent */}
        <aside className={`flex flex-col border-l bg-white transition-all duration-300 ${isExportVisible ? 'w-[450px]' : 'w-0 overflow-hidden shadow-none'}`}>
          <div className="p-4 border-b flex items-center justify-between shrink-0 bg-slate-50/50">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-[10px] uppercase tracking-widest text-slate-500">SVG Output</h2>
            </div>
            <button 
          ...
          <div className="flex-1 overflow-auto p-6 bg-slate-950 font-mono text-[11px] leading-relaxed text-indigo-100/80 selection:bg-indigo-500/30">
            <pre className="whitespace-pre-wrap break-all">
              {generatedSvgString}
            </pre>
          </div>

          <div className="p-6 bg-white border-t border-slate-100 space-y-3 shrink-0 shadow-[0_-4px_12px_-4px_rgba(0,0,0,0.1)]">
            <div className="flex gap-2">
              <button
                onClick={copyToClipboard}
                className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 rounded-xl font-bold transition-all shadow-sm ${
                  copied 
              ...
              <button
                onClick={downloadSvg}
                className="py-3 px-4 flex items-center justify-center rounded-xl font-bold text-slate-600 bg-white hover:bg-slate-50 transition-all border border-slate-200 active:scale-[0.98]"
                title="Download SVG"
              >
                <Download size={18} />
              </button>
      ...
      {!isExportVisible && (
        <button
          ...
          className="fixed bottom-8 right-8 p-5 bg-indigo-600 text-white rounded-full shadow-2xl hover:bg-indigo-700 transition-all active:scale-90 z-20 flex items-center justify-center"
        >
          <Terminal size={24} />
        </button>
      )}
